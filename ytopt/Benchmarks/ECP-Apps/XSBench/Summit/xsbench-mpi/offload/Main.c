...
int main( int argc, char* argv[] )
{
	...
	MPI_Comm_rank(MPI_COMM_WORLD, &mype);
        ...
        //wuxf:  set the env variables for thread affinity
...
unsigned long long run_event_based_simulation(Inputs in, SimulationData SD, int mype)
{
	...
	#pragma omp target teams distribute parallel for simd device(offloaded_to_device)\
	...
	reduction(+:verification) schedule(static,1) 
