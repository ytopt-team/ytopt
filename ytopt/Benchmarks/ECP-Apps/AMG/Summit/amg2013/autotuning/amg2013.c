   ...
   #pragma clang unrolling full
         ...
         if ( (m[0] * m[1] * m[2]) > 1)
         {
            ...
            for (box = 0; box < pdata.nboxes; box++)
            {
               ...
               i = box;
#pragma clang loop(r, q, p) tile sizes(32, 32, 32)
#pragma clang loop id(r)
               for (r = 0; r < m[2]; r++)
               {
#pragma clang loop id(q)
                  for (q = 0; q < m[1]; q++)
                  {
...
#pragma clang loop(k, j, i) tile sizes(32, 32, 32)
      ...
      char *argv[] )
{
   ...
   MPI_Comm_rank(MPI_COMM_WORLD, &myid);
       ...
       // Set number of OpenMP Threads -- wuxf
	int nthreads = 1;
   ...
   for (part = 0; part < data.nparts; part++)
   {
      ...
      /* set stencil values */
#pragma clang loop(var, i) tile sizes(32, 32)
#pragma clang loop id(var)
      for (var = 0; var < pdata.nvars; var++)
      {
...
#pragma clang loop(entry, j) tile sizes(32, 32)
#pragma clang loop id(entry)
...
#pragma clang loop id(j)
   ...
   #pragma omp parallel for
      ...
      {
         pdata = data.pdata[part];
#pragma clang loop(var, box) tile sizes(32, 32)
#pragma clang loop id(var)
         for (var = 0; var < pdata.nvars; var++)
         {
#pragma clang loop id(box)
            for (box = 0; box < pdata.nboxes; box++)
            {
               GetVariableBox(pdata.ilowers[box], pdata.iuppers[box], var,
                              ilower, iupper);
...
#pragma clang loop id(box)
   ...
   end_time = MPI_Wtime();
   printf("MPI Rank: %d; Total time: %f\n", myid, end_time-start_time);
