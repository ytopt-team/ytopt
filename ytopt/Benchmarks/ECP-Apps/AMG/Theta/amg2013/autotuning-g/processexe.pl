...
$filename1 =  $ARGV[0];
    ...
    while (<TEMFILE>) {
        ...
        if ($line =~ /system/) {
                print OUTFILE "    system(\"geopmlaunch aprun -n 4096 -N 1 --geopm-ctl=pthread --geopm-report gm.report -- ", "\$filename -laplace -P 16 16 16 -n 100 100 100 > tmpoutfile.txt 2>&1\");", "\n";
	} else {
                print OUTFILE $line, "\n";
        }
    }
