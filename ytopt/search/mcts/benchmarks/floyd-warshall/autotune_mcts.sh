...
(cd "${SCRIPTPATH}" && python3 "${ROOTPATH}/mctree_mcts.py" --packing-arrays=path autotune --outdir="${SAVEPATH}/${BASENAME}_mcts" --polybench-time \
  ...
  *.c -o "${BASENAME}" \
  -I"${SCRIPTPATH}" \
  -DLARGE_DATASET -mllvm -polly-only-func=kernel_floyd_warshall \
  )
