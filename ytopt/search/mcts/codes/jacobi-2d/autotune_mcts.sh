...
(cd "${SCRIPTPATH}" && python3 "${ROOTPATH}/mctree_mcts.py" --packing-arrays=A,B autotune --outdir="${SAVEPATH}/${BASENAME}_mcts" --polybench-time \
  ...
  *.c -o "${BASENAME}" \
  -I"${SCRIPTPATH}" \
  -DLARGE_DATASET -mllvm -polly-only-func=kernel_jacobi_2d \
  )
