...
cd "${SCRIPTPATH}"
"${CLANG_PREFIX}/bin/clang" -I"${CLANG_PREFIX}/projects/openmp/runtime/src" -I"${CLANG_PREFIX}/runtimes/runtimes-bins/openmp/runtime/src" -L"${CLANG_PREFIX}/runtimes/runtimes-bins/openmp/runtime/src" -flegacy-pass-manager -mllvm -polly-position=early -O3 -march=native -mllvm -polly -mllvm -polly-parallel -fopenmp -mllvm -polly-omp-backend=LLVM -mllvm -polly-scheduling=static -L"${CLANG_PREFIX}/lib" -L"${CLANG_PREFIX}/runtimes/runtimes-bins/openmp/runtime/src" -DLARGE_DATASET -DPOLYBENCH_TIME *.c -o "${BASENAME}_o3p" -I"${SCRIPTPATH}" -lm -mllvm -polly-allow-nonaffine -mllvm -polly-allow-nonaffine-branches -DNDEBUG -DDOUBLE 
