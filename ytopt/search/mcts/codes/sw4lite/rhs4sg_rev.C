...
extern "C" {
 ...
 void rhs4sg_rev_kernel(long ifirst, long jfirst, long kfirst, //long k1, long k2,
   ...
   long ni = I;
   long ilast = ifirst + I - 1;
   long jlast = jfirst + J - 1;
   long klast = kfirst + K - 1;
   //long klast = kfirst + nk - 1;
   //const int ni    = ilast-ifirst+1;
   const long nij   = I*J;
   const long nijk  = nij*K;
   //const int base  = -(ifirst+ni*jfirst+nij*kfirst);
   //const int base3 = base-nijk;
   const long nic  = 3*ni;
   const long nijc = 3*nij;
   const long ifirst0 = ifirst;
   const long jfirst0 = jfirst;
   const long kfirst0 = kfirst;
...
#define lu(c,i,j,k)     a_lu[i+ni*(j)+nij*(k)+nijk*(c)]   
#define strx(i)         a_strx[i]
#define stry(j)         a_stry[j]
#define strz(k)         a_strz[k]
#define acof(i,j,k)     a_acof[(i-1)+6*(j-1)+48*(k-1)]
#define bope(i,j)       a_bope[i-1+6*(j-1)]
#define ghcof(i)        a_ghcof[i-1]

   for(long k= k1; k <= k2 ; k++ )
      for(long j=jfirst+2; j <= jlast-2 ; j++ )
      	 for(long i=ifirst+2; i <= ilast-2 ; i++ ) {
		 ...
		 float_sw4* __restrict__ a_strz )
{
 ...
 //  std::cerr << "onesided[4] = " << onesided[4] << "\n";
  // std::cerr << "onesided[5] = " << onesided[5] << "\n";

#if 1
        assert(onesided[4] == 1 && "rhs4sg_rev_kernel assumes onesided[4] == 1");
        assert(onesided[5] == 0 && "rhs4sg_rev_kernel assumes onesided[5] == 0");

        int J = jlast-jfirst+1;
        int K = klast-kfirst+1;
       // std::cerr << "nk = " << nk << "\n";
