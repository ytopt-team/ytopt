...
(cd "${SCRIPTPATH}" && python3 "${ROOTPATH}/mctree_mcts.py" --packing-arrays=A,B,C autotune --outdir="${SAVEPATH}/${BASENAME}_mcts" --polybench-time \
  ...
  gemm.c polybench.c -o "${BASENAME}" \
  -I"${SCRIPTPATH}" \
  -DLARGE_DATASET -mllvm -polly-only-func=kernel_gemm \
  )
