...
(cd "${SCRIPTPATH}" && python3 "${ROOTPATH}/mctree_mcts.py" --packing-arrays=A,B,tmp,x,y autotune --outdir="${SAVEPATH}/${BASENAME}_mcts" --polybench-time \
  ...
  *.c -o "${BASENAME}" \
  -I"${SCRIPTPATH}" \
  -DLARGE_DATASET -mllvm -polly-only-func=kernel_gesummv \
  )
