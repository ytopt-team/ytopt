...
def run_exec_o3p(experiment,cwd,exefile,execopts):
#     print("Time the output...")
    class Exp:
        def __init__(self):
            self.nestexperiments = []
...
#             experiment.duration = datetime.timedelta(seconds=float(p.stdout.rstrip().splitlines()[-1]))
            ...
            print(f"Execution completed in {p.walltime}; polybench measurement: {polybench_measurement}")
#         experiment.duration = timedelta(seconds=median(experiment.durations)) 
        print(f"Execution completed in median {median(experiment.durations)} " )
    elif perf_time == True:
        for _ in range(N_repeat):                    
            cmd = ['perf','stat','--', str(exefile)]
            completed = subprocess.run(cmd,capture_output=True)
...
#         exit(errcode)
    ...
    execopts = argparse.Namespace(args=[], ld_library_path='/gpfs/jlse-fs0/users/jkoo/sw/clang13.2/llvm-project/release_pragma-clang-loop/lib:/gpfs/jlse-fs0/users/jkoo/sw/clang13.2/llvm-project/release_pragma-clang-loop/runtimes/runtimes-bins/openmp/runtime/src', polybench_time=True, timeout=None)
    expdir = Path(sys.argv[1]) # Path('/tmp/mctree-sduprtu7/experiment10/')
    exefile = expdir / sys.argv[2] # 
    print ('0',execopts)
    print ('1',expdir)
    print ('2',exefile)    
    run_exec_o3p(experiment=None,cwd=expdir,exefile=exefile,execopts=execopts)
