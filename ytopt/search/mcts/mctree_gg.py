...
def autotune(parser, args):
    ...
    if args:
        ...
        with contextlib.ExitStack() as stack:
            ...
            csvfile_gg = d / 'experiments_gg.csv'
            csvlog_gg = csvfile_gg.open('w+')             
            ...
            for p_v in range(maxdepth):
                try:
                    p_val.append(root.nestexperiments[0].pragmalist[p_v])
                except IndexError:
                    p_val.append("")    
            List_name.append(p_val)
            csvlog_gg.write(f"{idx},{exp_idx},{0},")
            for _ in range(N_repeat):
                csvlog_gg.write(f"{root.durations[_]},")
            csvlog_gg.write(f"{median(root.durations)},")
            try:    
                file1 = open(str(root.exppath) + "/cc.txt","r")
                cmdlines = file1.readlines()[0][1:-2]
                file1.close()
            except FileNotFoundError:
                cmdlines=" " 
            csvlog_gg.write(cmdlines+",") 
            for _ in range(maxdepth):
                csvlog_gg.write(List_name[-1][_])
            csvlog_gg.write("\n")
            csvlog_gg.flush()
            idx += 1 
            exp_idx += 1
            ...
            while not pq.empty():
                ...
                if item.duration == None:
                    ...
                    csvlog_gg.write(f"{idx},{exp_idx},{item.depth},")
                    for _ in range(N_repeat):
                        if item.duration != math.inf:
                            csvlog_gg.write(f"{item.durations[_]},")
                        else:
                            csvlog_gg.write(f"{execopts.timeout},")
                    if item.duration != math.inf:
                        csvlog_gg.write(f"{median(item.durations)},")
                    else:
                        csvlog_gg.write(f"{execopts.timeout},")
                    try:    
