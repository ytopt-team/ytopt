...
class Actor(Model):
    ...
    def __call__(self, obs, reuse=False):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            x = tf.layers.dense(x, self.nb_actions, kernel_initializer=tf.random_uniform_initializer(
...
class Critic(Model):
    ...
    def __call__(self, obs, action, reuse=False):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            x = tf.layers.dense(x, 1, kernel_initializer=tf.random_uniform_initializer(
                minval=-3e-3, maxval=3e-3), name='output')
        return x
