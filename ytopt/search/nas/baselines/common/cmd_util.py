...
from ytopt.search.nas.baselines.bench import Monitor
from ytopt.search.nas.baselines.common import set_global_seeds
from ytopt.search.nas.baselines.common.vec_env.subproc_vec_env import SubprocVecEnv
from ytopt.search.nas.baselines.common.vec_env.dummy_vec_env import DummyVecEnv
...
def make_env(env_id, env_type, mpi_rank=0, subrank=0, seed=None, reward_scale=1.0, gamestate=None, flatten_dict_observations=True, wrapper_kwargs=None, logger_dir=None):
    ...
    env = Monitor(env,
                  logger_dir and os.path.join(
                      logger_dir, str(mpi_rank) + '.' + str(subrank)),
...
def make_mujoco_env(env_id, seed, reward_scale=1.0):
    ...
    env.seed(seed)
    if reward_scale != 1.0:
        from ytopt.search.nas.baselines.common.retro_wrappers import RewardScaler
...
def common_arg_parser():
    ...
    parser.add_argument(
        '--env_type', help='type of environment, used when the environment type cannot be automatically determined', type=str)
    ...
    parser.add_argument(
        '--num_env', help='Number of environment copies being run in parallel. When not specified, set to number of cpus for Atari, and to 1 for Mujoco', default=None, type=int)
    parser.add_argument(
...
def robotics_arg_parser():
    ...
    parser = arg_parser()
    parser.add_argument('--env', help='environment ID',
                        type=str, default='FetchReach-v0')
    parser.add_argument('--seed', help='RNG seed', type=int, default=None)
