...
from ytopt.search.nas.baselines.common import explained_variance
from ytopt.search.nas.baselines.common import set_global_seeds
from ytopt.search.nas.baselines.common.tf_util import display_var_info
from ytopt.search.nas.baselines.ppo2.policies import build_ppo_policy
from ytopt.search.nas.baselines.ppo2.runner import Runner
...
def learn(*, network, env, total_timesteps, eval_env=None, seed=None, nsteps=128, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if model_fn is None:
        from ytopt.search.nas.baselines.ppo2.model import Model
        model_fn = Model
    ...
    allvars = tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES, scope=model.name)
    display_var_info(allvars)

    # Instantiate the runner object
    runner = Runner(env=env, model=model, nsteps=nsteps,
                    gamma=gamma, ob_space=ob_space, lam=lam)

    if eval_env is not None:
        eval_runner = Runner(env=eval_env, model=model,
                             nsteps=nsteps, gamma=gamma, ob_space=ob_space, lam=lam)
