...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        prev_transition = {
            'next_states': self.model.initial_state} if self.model.initial_state is not None else {}
        epinfos = []
        ...
        for _ in range(self.nsteps):
            ...
            self.obs, transitions['rewards'], self.dones, infos = self.env.step(
                transitions['actions'])
    ...
    def advantage_and_returns(self, values, rewards, dones, last_values, last_dones, gamma, use_non_episodic_rewards=False):
        ...
        for t in reversed(range(self.nsteps)):
            ...
            advantages[t] = lastgaelam = delta + gamma * \
                self.lam * next_non_terminal * lastgaelam
