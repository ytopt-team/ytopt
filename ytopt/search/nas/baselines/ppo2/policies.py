...
from ytopt.search.nas.baselines.a2c.utils import fc
from ytopt.search.nas.baselines.common import tf_util
from ytopt.search.nas.baselines.common.distributions import make_pdtype
from ytopt.search.nas.baselines.common.input import observation_placeholder, encode_observation
from ytopt.search.nas.baselines.common.models import get_network_builder
from ytopt.search.nas.baselines.common.tf_util import adjust_shape
from ytopt.search.nas.baselines.ppo2.layers import RNN
    ...
    def feed_dict(self, **kwargs):
        ...
        for key in kwargs:
            if key in self.step_input:
                feed_dict[self.step_input[key]] = adjust_shape(
                    self.step_input[key], kwargs[key])
        return feed_dict
...
def build_ppo_policy(env, policy_network, value_network=None, estimate_q=False, **policy_kwargs):
    ...
    def policy_fn(nbatch=None, nsteps=None, sess=None, observ_placeholder=None):
        ...
        with tf.variable_scope('current_rnn_memory'):
            ...
            policy_memory_size = policy_network.memory_size if isinstance(
                policy_network, RNN) else 0
            value_memory_size = value_network_.memory_size if isinstance(
                value_network_, RNN) else 0
