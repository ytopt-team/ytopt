...
class MicrobatchedModel(Model):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        self.grads_ph = [tf.placeholder(
            dtype=g.dtype, shape=g.shape) for g in self.grads]
        grads_ph_and_vars = list(zip(self.grads_ph, self.var))
        self._apply_gradients_op = self.trainer.apply_gradients(
            grads_ph_and_vars)
    ...
    def train(self,
        ...
        for microbatch_idx in range(self.nmicrobatches):
            _sli = range(microbatch_idx * self.microbatch_size,
        ...
        feed_dict = {ph: sum_g / self.nmicrobatches for ph,
                     sum_g in zip(self.grads_ph, sum_grad_v)}
