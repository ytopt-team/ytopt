...
from ytopt.search.nas.baselines.common import tf_util as U
...
class TransitionClassifier(object):
    def __init__(self, env, hidden_size, entcoeff=0.001, lr_rate=1e-3, scope="adversary"):
        ...
        # Build accuracy
        generator_acc = tf.reduce_mean(tf.to_float(
            tf.nn.sigmoid(generator_logits) < 0.5))
        expert_acc = tf.reduce_mean(tf.to_float(
            tf.nn.sigmoid(expert_logits) > 0.5))
        ...
        # Loss + Accuracy terms
        self.losses = [generator_loss, expert_loss, entropy,
