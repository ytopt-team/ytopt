...
from ytopt.search import Search, util
from ytopt.evaluator.evaluate import Encoder
from ytopt.search.nas.baselines import logger
from ytopt.search.nas.baselines.common.cmd_util import (common_arg_parser,
                                                            make_env,
                                                            make_vec_env,
                                                            parse_unknown_args)
from ytopt.search.nas.baselines.common.tf_util import get_session
from ytopt.search.nas.baselines.common.vec_env import (VecEnv,
                                                           VecFrameStack,
                                                           VecNormalize)
from ytopt.search.nas.baselines.common.vec_env.vec_video_recorder import \
    VecVideoRecorder
from ytopt.search.nas.env.neural_architecture_envs import \
    NeuralArchitectureVecEnv
from ytopt.search.nas.utils._logging import JsonMessage as jm
...
class NeuralArchitectureSearch(Search):
    ...
    Args:
        problem (str): Module path to the Problem instance you want to use for the search (e.g. ytopt.benchmark.nas.linearReg.Problem).
        run (str): Module path to the run function you want to use for the search (e.g. ytopt.search.nas.model.run.quick).
    ...
    def _extend_parser(parser):
        parser.add_argument("--problem",
                            type=str,
                            default="ytopt.benchmark.nas.linearReg.Problem",
                            help="Module path to the Problem instance you want to use for the search (e.g. ytopt.benchmark.nas.linearReg.Problem)."
                            )
        parser.add_argument("--run",
                            type=str,
                            default="ytopt.search.nas.model.run.quick",
                            help="Module path to the run function you want to use for the search (e.g. ytopt.search.nas.model.run.quick)."
                            )
...
def get_alg_module(alg, submodule=None):
    ...
    alg_module = import_module(
        '.'.join(['ytopt.search.nas.baselines', alg, submodule]))
    try:
