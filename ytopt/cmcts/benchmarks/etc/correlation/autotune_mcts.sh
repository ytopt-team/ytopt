...
(cd "${SCRIPTPATH}" && python3 "${ROOTPATH}/mctree.py" --packing-arrays=data,corr,mean,stddev autotune --polybench-time \
  ...
  *.c -o "${BASENAME}" \
  -I"${SCRIPTPATH}" -lm \
  -DLARGE_DATASET -mllvm -polly-only-func=kernel_correlation \
  )
