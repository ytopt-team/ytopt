...
class Plopper:
    ...
    def findRuntime(self, x, params):
        ...
        commonflags = f"""-DLARGE_DATASET -DPOLYBENCH_TIME -I{utilities_dir} -I{kernel_dir} {interimfile} {utilities_dir}/polybench.c -o {tmpbinary} -lm -g """

        #original script
        
        #cmd1 = "clang -O2 -fopenmp -fopenmp-targets=nvptx64 -DLARGE_DATASET -DPOLYBENCH_TIME -Xopenmp-target -march=sm_60 -I"+utilities_dir +  " -I"+kernel_dir+" "+interimfile+" "+utilities_dir+"/polybench.c -o "+tmpbinary+" -lm"
        
        cmd1 = f"""invoke_test clang -O2 -fopenmp -fopenmp-targets=nvptx64 -Xopenmp-target -march=sm_60 {commonflags} -I/usr/tce/packages/cuda/cuda-10.1.168/include -L/usr/tce/packages/cuda/cuda-10.1.168/lib64 -Wl,-rpath=usr/tce/packages/cuda/cuda-10.1.168/lib64 -lcudart_static -ldl -lrt -pthread"""

        cmd2 = f"""invoke_test {utilities_dir}/time_benchmark.sh {tmpbinary}"""
