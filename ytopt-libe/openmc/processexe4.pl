...
$filename1 =  $ARGV[0];
    ...
    while (<TEMFILE>) {
        ...
        if ($line =~ /system/) {
                print OUTFILE "    system(\"srun -N 4 -n ", $ARGV[2]*8*4, " --ntasks-per-gpu=", $ARGV[2], " --gpus-per-node=8 --gpu-bind=closest -c ", $ARGV[1], " --cpu-bind=", $ARGV[3]," sh \$filename > tmpoutfile.txt 2>&1\");", "\n";
	} else {
                print OUTFILE $line, "\n";
        }
    }
